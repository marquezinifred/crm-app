    ...
    <main className="p-6 md:p-10 max-w-5xl mx-auto space-y-8">
      ...
      <section className="border rounded-lg p-5 bg-white">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div>
            <div className="text-xs uppercase text-neutral-500 tracking-wide">Plano atual</div>
          ...
          <div className="flex gap-2 flex-wrap">
            {status.data?.stripeCustomerId && (
              <button
                ...
                className="px-4 py-2 rounded-md border hover:bg-neutral-50 disabled:opacity-50"
              >
      ...
      <section className="grid md:grid-cols-3 gap-4">
        {(['STARTER', 'PRO', 'ENTERPRISE'] as const).map((plan) => (
          <article key={plan} className="border rounded-lg p-5 bg-white">
            <div className="text-base font-semibold">{plan}</div>
            <ul className="text-sm text-neutral-700 mt-3 space-y-1">
              {plan === 'STARTER' && (
                <>
                  <li>10 usuários</li>
              ...
              ([label, ch, fmt]) => (
                <div key={label}>
                  <div className="flex justify-between text-sm">
                    ...
                    <span className={ch.exceeded ? 'text-rose-700' : 'text-neutral-600'}>
        ...
        <ul className="text-sm divide-y">
          {history.data?.map((ev) => (
            <li key={ev.id} className="py-2 flex justify-between gap-3">
              <span>
                <span className="font-mono text-xs px-2 py-0.5 rounded bg-neutral-100">
                  {ev.type}
                </span>
                {ev.error && <span className="ml-2 text-rose-700 text-xs">erro: {ev.error}</span>}
              </span>
